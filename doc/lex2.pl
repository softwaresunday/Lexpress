...
lexpress -->   unwrap,
               ...
               essential_primes,
                 iterate([
                     step( init_cost,   reduce,      reduce  ),
                     step( reduce,      expand,      done    ),
                     step( expand,      irredundant, reduce2 ),
                     step( irredundant, reduce,      reduce2 ),
                     step( reduce2,     init_cost,   done    )
                 ]),
               add_to_care,
               sub_from_dont_care,
               make_sparse.
...
%<h3>iterate//1</h3>
%<code>
iterate([step(Step,E,NE)|T]) -->
            iterate(Step,[step(Step,E,NE)|T]).
...
iterate(Step, Steps) -->
		   ...
		   { sum_costs(PLA, cost(0, 0, 0), Current),
		     Previous = Current -> member(step(Step,Next,_),Steps)
				        ;  member(step(Step,_,Next),Steps)
		   },
		   iterate(Next, Steps).
%</code>
%<h3>iterate//1</h3>
